...
public class JPADaoGenerico<T, PK extends Serializable> implements
	...
	// Deve-se ter cuidado, pois deve ser usado no inicio do processo de acesso ao
	// recurso critico.
	...
	public final T getPorIdComLock(PK id) throws ObjetoNaoEncontradoException {
		...
		try {
			...
			if (t != null) {
				em.lock(t, LockModeType.READ); // read: define o tipo de lock como sendo de leitura.
				em.refresh(t);
			} else {
				throw new ObjetoNaoEncontradoException();
			}
	...
	public final T busca(Method metodo, Object[] argumentos)
		...
		T t = null;
		try {
			String nomeDaBusca = getNomeDaBuscaPeloMetodo(metodo); // ex: Familia.recuperaUmaFamiliaEModelos
			...
			return t;
		} catch (NoResultException e)// Caso nao seja encontrado o objeto sera retornada esta excecao
		{
			throw new ObjetoNaoEncontradoException();
		} catch (RuntimeException e) {
			throw new InfraestruturaException(e);
